    ...
    <div className="flex items-center gap-1.5 px-4 py-2.5 border-b border-outline-variant/10 bg-surface-container-lowest/80 backdrop-blur-sm flex-shrink-0 overflow-x-auto">
      {conversations.map((conv) => {
        ...
        return (
          <button
            ...
            className={cn(
              'group flex items-center gap-1.5 py-1.5 rounded-lg text-xs font-medium transition-all duration-150 flex-shrink-0 max-w-[180px]',
              conversations.length > 1 ? 'pl-3 pr-1.5' : 'px-3',
              isActive
      ...
      <button
        ...
        className={cn(
          ...
          conversations.length === 1 ? 'w-8 h-8' : 'w-7 h-7',
          isBusy
      ...
      >
        <span className="material-symbols-outlined" style={{ fontSize: conversations.length === 1 ? '20px' : '18px' }}>add</span>
      </button>
    </div>
  );
}
