import type { JobAlerts } from '@/types';
        ...
        <div className="rounded-xl border border-red-200 bg-red-50 p-4">
          ...
          <ul className="space-y-1.5">
            {alerts.overdue.map((a) => (
              <li key={`${a.company}-${a.position}`} className="text-sm text-red-700">
                <span className="font-medium">{a.position}</span> at {a.company}
                {' — '}follow-up was due {a.follow_up_date} ({a.days_overdue} days overdue)
                {a.contact_email && (
                  <span className="text-red-600"> · {a.contact_email}</span>
                )}
              </li>
            ))}
          </ul>
        ...
        <div className="rounded-xl border border-amber-200 bg-amber-50 p-4">
          ...
          <ul className="space-y-1.5">
            {alerts.upcoming_7_days.map((a) => (
              <li key={`${a.company}-${a.position}`} className="text-sm text-amber-700">
                <span className="font-medium">{a.position}</span> at {a.company}
        ...
        <div className="rounded-xl border border-blue-200 bg-blue-50 p-4">
          ...
          <ul className="space-y-1.5">
            {alerts.interviews_upcoming.map((a) => (
              <li key={`${a.company}-${a.position}`} className="text-sm text-blue-700">
                <span className="font-medium">{a.position}</span> at {a.company}
                {' — '}{a.interview_date}
